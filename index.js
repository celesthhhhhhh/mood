...
OUTPUT (respond ONLY with raw JSON, no commentary, no markdown):`;
        ...
        if (currentProfileName) {
            ...
            const supportsJsonMode = ['openai', 'custom', 'openrouter', 'groq', 'mistral'].includes(cc_source);
            let generate_data = {
...
function parseAISongJSON(aiText) {
    ...
    // 2. First { ... } block containing "title"
    if (!parsed) {
        ...
        if (titleMatch) {
            ...
            for (let i = startIdx; i < aiText.length; i++) {
                if (aiText[i] === '{') depth++;
                else if (aiText[i] === '}') { depth--; if (depth === 0) { endIdx = i; break; } }
            }
            if (endIdx !== -1) {
    ...
    // 3. Quoted key-value: "Title": "...", "Artist": "..."
    if (!parsed) {
    ...
    // 4. Plain text: Title: X\nArtist: Y  or  Title - X / Artist - Y
    if (!parsed) {
        const titleMatch = aiText.match(/(?:^|\n)\s*[Tt]itle\s*[:\-]\s*(.+)/m);
        const artistMatch = aiText.match(/(?:^|\n)\s*[Aa]rtist\s*[:\-]\s*(.+)/m);
        if (titleMatch || artistMatch) {
            parsed = {
                Title: titleMatch ? titleMatch[1].trim().replace(/^["']|["']$/g, '') : "",
                Artist: artistMatch ? artistMatch[1].trim().replace(/^["']|["']$/g, '') : ""
            };
        }
    }

    // 5. Natural language: "Song Name" by Artist Name  or  play "Song" — Artist
    if (!parsed) {
        const byMatch = aiText.match(/[«"'"]([^"'"»]+)[«"''"]\s+by\s+([^\n.,!?]+)/i);
        if (byMatch) {
            parsed = { Title: byMatch[1].trim(), Artist: byMatch[2].trim() };
        }
    }

    // 6. Artist — Song (dash/hyphen pattern common in Russian)
    if (!parsed) {
        const dashMatch = aiText.match(/([^\n\-–—]+?)\s*[–—-]\s*([^\n\-–—]+)/);
        if (dashMatch && dashMatch[1].length < 60 && dashMatch[2].length < 60) {
            parsed = { Title: dashMatch[2].trim(), Artist: dashMatch[1].trim() };
        }
    }
